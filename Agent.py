...
class QLearningAgent:
    def __init__(self, filename, color):
        ...
        self.file_name = filename + "_" + color + ".json"
        self.actions = None
    ...
    for episode in range(num_episodes):
        ...
        while my_engine.winner == "None":
            if my_engine.turn_player == agent_color:
                ...
                next_state, score = my_engine.attempt_move(chosen_action[0], chosen_action[1], agent_color)
                ...
                agent.update_q_value(old_state, chosen_action, reward, next_state)
                reward_until_end += reward
            else:
        ...
        if my_engine.winner == "b":
            if agent_color == "b":
                total_reward = reward_until_end + 100
                agent.update_q_value(old_state, chosen_action, 100, current_state)
            else:
                total_reward = reward_until_end - 100
                agent.update_q_value(old_state, chosen_action, -100, current_state)
            b_wins += 1
...
def train_agent_vs_agent(board_choice, num_episodes):
    ...
    for episode in range(num_episodes):
        ...
        if my_engine.winner == "b":
            agent1.update_q_value(old_state_agent_1, chosen_action_agent_1, 100, current_state_agent_1)
            agent2.update_q_value(old_state_agent_2, chosen_action_agent_2, -100, current_state_agent_2)
            b_wins += 1
        elif my_engine.winner == "w":
            agent1.update_q_value(old_state_agent_1, chosen_action_agent_1, -100, current_state_agent_1)
            agent2.update_q_value(old_state_agent_2, chosen_action_agent_2, 100, current_state_agent_2)
